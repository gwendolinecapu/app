...
export class OpenAIProvider implements IImageProvider {
    ...
    async generateInfoImage(prompt: string, options?: ImageGenerationOptions): Promise<Buffer[]> {
        ...
        if (useResponsesApi && options?.referenceImages && options.referenceImages.length > 0) {
            ...
            options.referenceImages.slice(0, 5).forEach(base64 => {
                inputContent.push({
                    type: "input_image",
            ...
            const payload: any = {
                model: this.model, // e.g. 'gpt-image-1.5'
                input: [{
                    role: "user",
                ...
                tools: [{
                    ...
                    input_fidelity: "high", // Critical for rituals
                    action: "generate"
                }]
            };
            ...
            if (!imageGenerations.length) throw new Error("No image generated by Responses API");
