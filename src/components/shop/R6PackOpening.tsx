...
import R6AlphaPack from './R6AlphaPack';
import R6ItemReveal from './R6ItemReveal';
...
const TIER_CONFIG: Record<LootBoxTier, {
    ...
    elite: {
        ...
        bgGradient: ['#1A150A', '#2A1500', '#0A0500'],
    },
};
...
export default function R6PackOpening({ visible, tier, packCount = 1, onClose }: R6PackOpeningProps) {
    ...
    const bgParticles = useMemo(() =>
        Array.from({ length: 15 }, (_, i) => ({
            ...
            color: i % 2 === 0 ? config.primaryColor : 'rgba(255,255,255,0.2)',
        })), [tier]);
    ...
    useEffect(() => {
        if (visible && packCount > 0) {
            ...
            contentScale.value = withTiming(1, { duration: 350, easing: Easing.out(Easing.cubic) });
        } else {
            bgOpacity.value = 0;
                ...
                <SafeAreaView style={styles.safeArea}>
                    ...
                    <Animated.View style={[styles.content, contentStyle]}>
                            ...
                            >
                                ...
                                <Animated.View entering={FadeInUp.delay(600).duration(300)}>
                                    <TouchableOpacity
                                        ...
                                        activeOpacity={0.8}
                                    >
                                        <Text style={[styles.nextButtonText, tier === 'elite' && { color: '#1A1500' }]}>
                        ...
                        {phase === 'summary' && allResults.length > 0 && (
                            <Animated.View entering={FadeIn.duration(400)} style={styles.summaryContainer}>
                                <SummaryGrid
