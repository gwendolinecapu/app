...
export async function fetchActiveStories(
    ...
    // We take the top 10 relevant systems
    const uniqueSystemIds = Array.from(new Set(systemIds)).slice(0, 10);
    ...
    const q = query(
        storiesRef,
        where('system_id', 'in', uniqueSystemIds),
    ...
    // Apply Strict Privacy Filter if provided
    if (allowedAuthorIds && allowedAuthorIds.length > 0) {
        const allowedSet = new Set(allowedAuthorIds);
        docs = docs.filter(doc => {
            const data = doc.data();
            return allowedSet.has(data.author_id);
        });
    }
